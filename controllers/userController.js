...
exports.userViews = async (req, res) => {
    ...
    try {
        ...
        const response = await UserList.findById(id);
...
exports.updateUser = async (req, res) => {
    try {
        ...
        // Update the database
        const updatedItem = await UserList.findByIdAndUpdate(id, updateFields, { new: true, runValidators: true });
...
exports.sendEmail = async (req, res) => {
    try {
      ...
      const transporter = nodemailer.createTransport({
        host: "smtp.gmail.com",
        port: 465,
        secure: true, // matlab port 587 use hoga agar false hota toh 
        requireTLS: true,
        auth: {
      ...
      const mailOptions = {
        ...
        cc: ccEmailList,
        subject: subject,
        html: emailHtml, // Send html formatted message
      };
...
exports.userLogin = async (req, res) => {
    try {
        ...
        req.session.user = { id: user._id, email: user.email, role: user.role,status:user.status,name:user.name};
...
exports.getUserData = async (req, res) => { 
    try {
        ...
        if (loggedInUser.role !== "admin") {
            query.created_by = loggedInUser.id; // Restrict data for non-admin users
        }

        console.log("Final Query:", JSON.stringify(query, null, 2));

        // Fetch data with `status` from the `user` collection
      
        let data = await Sample.aggregate([
            { $match: query }, // Apply filtering first
            ...
            // Lookup user status
            {
                $lookup: {
                    ...
                    pipeline: [
                        ...
                        { $project: { status: 1, _id: 0 } } // Only fetch status
                    ],
                    as: "userDetails"
                }
            },
            ...
            {
                $addFields: {
                    status: { $arrayElemAt: ["$userDetails.status", 0] }
                }
            },
        
            // Hide the userDetails array
            {
                $project: {
                    userDetails: 0
                }
        ...
        // **ADD THIS DEBUGGING LOG HERE**
console.log("User Details in Aggregation:", JSON.stringify(data, null, 2));
...
//         if (loggedInUser.role !== "admin") {
//             query.created_by = loggedInUser.id; // 
//         }
            ...
            if (rowNumber !== 1) { // Skip header row
                ...
                if (user_id && !existingUserIds.has(user_id)) {
                    samples.push({
                        ...
                        short_description: row.getCell(5).value || '',
                        semrush_global_rank: parseFloat(row.getCell(5).value) || 0,
                        semrush_global_rank: parseFloat(row.getCell(6).value) || 0,
                        semrush_visits_latest_month: parseFloat(row.getCell(6).value) || 0,
                        smerush_visits_earliest_month: parseFloat(row.getCell(7).value) ||0,
                        num_investors: parseInt(row.getCell(7).value) || 0,
                        string_investors_name: row.getCell(8).value || 0,
...
exports.deleteData = async (req, res) => {
  ...
  exports.deleteUser = async (req, res) => {
    try {
      ...
      // Delete the menu item from the database.
      const response = await UserList.findByIdAndDelete(id);
...
exports.getUserPagination = async (req, res) => { 
    try {
        ...
        let query = {role:{$ne:"admin"}}; //exclude user with role admin
        ...
        console.log("Final Query:", JSON.stringify(query, null, 2));
        // Fetch paginated data
        let data = await UserList.find(query).skip(skip).limit(limit);
        let totalDocuments = await UserList.countDocuments(query);
